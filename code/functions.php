...
if (!defined('SUFFUSION_THEME_VERSION')) {
	define('SUFFUSION_THEME_VERSION', '4.2.1');
}
...
function suffusion_add_theme_supports() {
	...
	if (version_compare($wp_version, '3.3.2', '<=')) {
		...
		if (!isset($options) || !is_array($options) || $theme_name == 'root') {
			...
			if ($options['suf_header_style_setting'] == 'theme') {
				if ($theme_name == 'minima') {
					$options['suf_header_style_setting'] = 'out-cfull-halign';
				}
				else {
					$options['suf_header_style_setting'] = 'in-align';
				}
			}

			if ($options['suf_header_style_setting'] == 'out-cfull-halign') {
...
function suffusion_enqueue_skin_scripts() {
	...
	if (isset($suffusion_theme_name) && $suffusion_theme_name == 'photonique') {
		...
		wp_enqueue_style('suffusion-skin-fonts', "http://fonts.googleapis.com/css?family=Handlee", array(), null);
	}
}
...
function suffusion_customize_register($wp_customize) {
	...
	$wp_customize->add_control(new Suffusion_Customize_Image_Picker($wp_customize,
		...
		array(
			...
			'choices' => array(
				"scribbles" => array('src' => $template_uri.'/admin/images/photonique.jpg', 'alt' => 'Scribbles', 'style' => 'width: 150px;'),
				"photonique" => array('src' => $template_uri.'/admin/images/photonique.jpg', 'alt' => 'Photonique', 'style' => 'width: 150px;'),
				...
				"dark-theme-royal-blue" => array('src' => $template_uri.'/admin/images/Dark-theme-royal-blue.jpg', 'alt' => 'Royal on a dark theme', 'style' => 'width: 150px;'),
				"light-theme-gray-1" => array('src' => $template_uri.'/admin/images/Light-theme-gray-1.jpg', 'alt' => 'Gray Shade 1 on a light theme', 'style' => 'width: 150px;'),
	...
	wp.customize('suffusion_options[suf_color_scheme]', function(value) {
		value.bind(function(to) {
			...
			var fonts = {
				...
				'scribbles': 'http://fonts.googleapis.com/css?family=Handlee'
			};
